...
function getCarInfoById(inventory, id) {
  ...
  return `No car found with id number ${id}.`;
}
...
function sortCarInventory(inventory) {
 ...
 /* let output = [];
  output.push(inventory[0]);
  for(let i = 1; i < inventory.length; i++){
    let currentCar = inventory[i];
    if(currentCar.name<output[0].name)output.unshift(currentCar);
    else if(currentCar.name>output[output.length-1].name)output.push(currentCar);
    else {
      for(let j = 0; j<output.length-1; j++){
        if(currentCar.name>output[j].name&&currentCar.name<output[j+1].name){
          output.splice(j,0,currentCar);
        }
      }
...
function getModelYears(inventory) {
  let output = [];
  for(let i = 0; i < inventory.length; i++){
    output.push(inventory[i].car_year);
  }
  return output;
}
...
function getOlderCars(inventory, year) {
  let output = [];
  for (let i = 0; i < inventory.length; i++) {
    if(inventory[i].car_year<=year)output.push(inventory[i]);
  }
  return output;
}
...
function getGermanCars(inventory) {
  ...
  let output = [];
  for (let i = 0; i < inventory.length; i++) {
    if (germanManufacturers.includes(inventory[i].car_make)) output.push(inventory[i]);
  }
  return output;
}
...
const addFive = x=>x+5; // code here!
const argTimesTwo = x=>x*2; // code here!
...
*/
function carMaker(startingOdometer) {
  return {
